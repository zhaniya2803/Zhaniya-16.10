...
public class OrdersManager {
    ...
    String getMaxOrderCustomerName() {
        ...
        for (Map.Entry<String, ArrayList<Double>> many : customersOrders.entrySet()) {

            for (double m : many.getValue()) {
                if (m >= maxOrder) {
                    maxOrder = m;
                    customerName = many.getKey();
                }
            }
        }
        return customerName;
    ...
    void removeUnprofitableOrders() {
        ...
        for (String name : customersOrders.keySet()) {
            ...
            double ordersSum = 0;
            for (double sum : customersOrders.get(name)) {
                ordersSum += sum;
            }
            if (ordersSum < 5000) {
                names.add(name);
